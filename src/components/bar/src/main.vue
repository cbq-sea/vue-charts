...
export default {
  ...
  computed: {
    chartConfig() {
      ...
      const option = {
        ...
        xAxis: this.isHorizontal ? yConfig : xConfig,
        yAxis: this.isHorizontal ? xConfig : yConfig,
      ...
      option.series.forEach(item => {
        ...
        item.stack = item.stack || this.isStack ? '总量' : ''
  ...
  methods: {
    ...
    setAnimate(option) {
      ...
      const { xAxis = {} } = option
      if (!chartInstance || Array.isArray(xAxis) || this.isHorizontal || !this.loop) return
